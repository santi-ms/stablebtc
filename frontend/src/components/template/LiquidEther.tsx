...
function LiquidEther({
  ...
  // CRÍTICO: useEffect SEPARADO para actualizar props sin recrear todo
  useEffect(() => {
    ...
    if (webglRef.current.autoDriver) {
      webglRef.current.autoDriver.enabled = autoDemo;
      webglRef.current.autoDriver.speed = autoSpeed;
      webglRef.current.autoDriver.resumeDelay = autoResumeDelay;
      webglRef.current.autoDriver.rampDurationMs = autoRampDuration * 1000;
      if (webglRef.current.autoDriver.mouse) {
        webglRef.current.autoDriver.mouse.autoIntensity = autoIntensity;
        webglRef.current.autoDriver.mouse.takeoverDuration = takeoverDuration;
      }
    }
  ...
  useEffect(() => {
    if (!mountRef.current || webglRef.current) return;
    ...
    class MouseClass {
      constructor() {
        ...
        this.mouseThrottle = 16;
      }
      init(container) {
        this.container = container;
        container.addEventListener('mousemove', this._onMouseMove, { passive: true });
        ...
        container.addEventListener('mouseenter', this._onMouseEnter, { passive: true });
        container.addEventListener('mouseleave', this._onMouseLeave, { passive: true });
    ...
    const color_frag = `
      ...
      void main(){
        ...
        lenv = smoothstep(0.0, 1.2, lenv);
        lenv = pow(lenv, 0.7);
        
        float noise = sin(uv.x * 25.0) * cos(uv.y * 20.0) * 0.15;
        lenv += noise * 0.5;
        
        vec2 center = vec2(0.5, 0.5);
        float dist = length(uv - center);
        float radialGlow = exp(-dist * 2.5) * 0.4;
        lenv += radialGlow;
        
        float palettePos = clamp(lenv * 0.9, 0.0, 1.0);
        vec3 c = texture2D(palette, vec2(palettePos, 0.5)).rgb;
        
        vec3 glowColor = c * 1.6;
        
        vec3 outRGB = mix(bgColor.rgb, glowColor, lenv * 0.9);
        float outA = mix(bgColor.a, 0.95, smoothstep(0.0, 0.8, lenv));
        
        outRGB = mix(outRGB, c * 1.4, lenv * 0.5);
  ...
  }, [colors]); // CRÍTICO: Solo colors en dependencias

  return <div ref={mountRef} className={`liquid-ether-container ${className || ''}`} style={style} />;
}

// CRÍTICO: Export con memo
